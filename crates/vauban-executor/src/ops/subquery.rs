//! `SubqueryEval`: the evaluation of the subqueries of an expression. The operator is not written yet; its entry point answers the
...
pub(crate) fn build<'a>(plan: &PhysicalPlan) -> SqlResult<Box<dyn Operator<'a> + 'a>> {
    let _ = plan;
    Err(not_implemented("SubqueryEval"))
}
